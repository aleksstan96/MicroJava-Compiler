...
public class CodeGenerator extends VisitorAdaptor {
	...
	private boolean negFirstTerm = false;
	private boolean factExpr = false;
	private int factorExprCount = 0;
	...
	public void visit(FactorNew factorNew) { //pravimo novi niz
	...
	public void visit(ProgName progName) {
		...
		for (Obj obj : predefinedMethods) { //kao da smo ih definisali na pocetku programa
			obj.setAdr(Code.pc);
			Code.put(Code.enter);
	...
	public void visit(MethodTypeNameVoid methId) {
		...
		Code.put(Code.enter); //enter zahteva kao parametre: level(broj parametara) i broj lokalnih promenljivih za metodu
		Code.put(methId.obj.getLevel()); 
		Code.put(methId.obj.getLocalSymbols().size());
	}
	
	public void visit(Term term) {	
		if (negFirstTerm && factorExprCount == 0) {
			Code.put(Code.neg);
			negFirstTerm = false;
		}
	}
	...
	public void visit(FactorExprBegin factorExprBegin) {
		if (negFirstTerm) {
			factorExprCount++;
			factExpr = true;
		} 
...
//		}
	...
	public void visit(DesginatorStmtPlusPlus pp) {
		...
		Code.load(obj); //ako je Elem radi aload
		Code.loadConst(1);
		Code.put(Code.add);
		Code.store(obj); //ako je Elem radi astore
	}
